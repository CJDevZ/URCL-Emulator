...
execute store result score #1 urcl.math run function urcl:run/arg/get/pc0 with storage urcl:temp
scoreboard players operation $op_code urcl.runtime += $op_code urcl.runtime

execute store result score #2 urcl.math run function urcl:run/arg/get/pc1 with storage urcl:temp
scoreboard players operation $op_code urcl.runtime += $op_code urcl.runtime

execute store result storage urcl:temp pointer int 1 run scoreboard players operation #1 urcl.math += #2 urcl.math
execute store result storage urcl:temp value int 1 run function urcl:run/arg/get/pc2 with storage urcl:temp
